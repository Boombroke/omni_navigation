...
[IMU (BMI088)] --+                 |               |              |
                          [odom_bridge]             |              v
[先验 PCD 地图] ---------> [Small GICP 重定位]      |       [MPPIController (Omni)]
                           (map → odom 修正)        |              |
                                                    |              v
[裁判系统 (Referee)] ----> [sentry_behavior 状态机]  <----+      [Velocity Smoother]
                           (高层战术决策)                           |
                                                                    v
                                                          [Fake Vel Transform]
                                                          (gimbal_yaw_fake→gimbal_yaw 坐标旋回, 解耦云台)
                                                                    |
                                                                    v
...
- `chassis_odometry`（odom 系，child=base_footprint，twist 线速度在 odom 惯性轴、angular.z=底盘 yaw 率；独立 prev 状态 + 消息时间戳算 dt；供 MPPI 速度反馈，解耦云台瞄准）
...
> **关键设计**：底盘持续自旋时，`gimbal_yaw` 实时变化；`gimbal_yaw_fake` 以相同角速度反向旋转，使 Nav2 规划目标点在惯性系中保持稳定。`fake_vel_transform` 在执行端把 Nav2 输出从 `gimbal_yaw_fake` 旋回真实的 `gimbal_yaw` 系，解耦云台独立瞄准；ROS 不再叠加自旋，物理自旋（若有）由 MCU 固件负责。
...
系统使用 `nav2_smac_planner::SmacPlanner2D`，即基于 A* 的 2D 网格规划器。全向底盘不需要考虑最小转弯半径约束，SmacPlanner2D 在全向移动场景下路径更简洁、计算开销更低。开启 `downsample_costmap`（factor=2）后规划提速 2-4×；`smoother` 启用 `do_refinement` 使路径更柔和。
...
- **配置参数**:
    ...
    | `downsample_costmap` | true | 对代价地图 2× 下采样，规划提速 2-4×（factor=2） |
    | `max_iterations` | 1000000 | 最大搜索迭代次数，-1 禁用 |

### 4.2 局部控制器 - MPPIController (Omni)
系统使用 `nav2_mppi_controller::MPPIController`（`motion_model: "Omni"`），以预测性采样方式在全向底盘上实现平滑避障跟踪。`omni_pid_pursuit_controller` 包保留作回退（git revert 可切回），默认控制器已切换为 MPPI。

- **核心逻辑**：在预测窗口内随机采样速度轨迹，按代价（路径跟踪 + 障碍物 + 约束）加权平均得到平滑控制输出，无需显式前瞻距离或 PID 调参。
- **朝向策略**：全向底盘朝向 don't-care；去掉航向类 critics（PathAngleCritic 等），仅保留 ConstraintCritic / CostCritic / GoalCritic / PathAlignCritic / PathFollowCritic；`wz_max` 设低（0.5 rad/s）以减少无效转向。
- **速度反馈**：`odom_topic` 设为 `chassis_odometry`（base_footprint 系，twist 解耦云台瞄准），避免因云台旋转引入虚假角速度。
- **关键参数**:
    | 参数名 | 默认值 | 说明 |
    |--------|--------|------|
    | `motion_model` | `"Omni"` | 全向底盘运动模型 |
    | `vx_max` | 2.0 | 最大 x 线速度 (m/s) |
    | `vy_max` | 2.0 | 最大 y 线速度 (m/s) |
    | `wz_max` | 0.5 | 最大角速度 (rad/s)，低权限减少无效转向 |
    | `odom_topic` | `chassis_odometry` | 速度反馈话题（底盘系，解耦云台） |
    | critics | 见上文 | ConstraintCritic / CostCritic / GoalCritic / PathAlignCritic / PathFollowCritic |
...
- **功能**: 最终的指令转换层。
- **坐标补偿**: Nav2 在 `gimbal_yaw_fake`（不随云台旋转的虚拟惯性系）下规划直线，避免云台旋转时 Nav2 看到底盘"原地打转"。该模块根据 TF 实时查询 `gimbal_yaw_fake → gimbal_yaw` 的相对偏转角，把 `cmd_vel_nav2_result` 的线速度旋回真实的 `gimbal_yaw` 底盘系，输出到 `/cmd_vel_chassis`（`geometry_msgs/Twist`），由 `rm_serial_driver` 订阅。MCU 固件按 `gimbal_yaw` 系解释 `cmd_vel`，ROS 侧不再叠加自旋；物理自旋（若有）由 MCU 固件负责。
- **关键参数**: `fake_robot_base_frame: "gimbal_yaw_fake"`、`robot_base_frame: "gimbal_yaw"` 决定坐标补偿的源/目标系。`cmd_spin` 话题、`init_spin_speed` 参数及自旋叠加逻辑已移除。

### 4.5 跟随模式 (Follow Mode)
`sentry_behavior` 新增 FOLLOW 最高优先父 guard，在有效、未过期的 `TrackGoal` 到达时抢占战术层，失效/超时后自动回落到战术 guard 表。

- **消息**: `rm_interfaces/msg/TrackGoal`（`Header frame_id=chassis`，字段 `rel_x / rel_y / rel_yaw / valid`，车体系相对位姿）。
- **处理流程**: 节点订阅 `vision/target_body`（默认，可由 `target_topic` 参数覆盖）→ tf2 查询 `chassis→map` 转换相对位姿到 map 系 → standoff 逻辑（距离 > `follow_standoff_dist + follow_deadband` 则趋近，< `follow_standoff_dist - follow_deadband` 则后退）→ 发布 `PoseStamped` 到 `/goal_pose`。
- **回落条件**: `valid=false`、消息时间戳超过 `follow_staleness_sec`（默认 0.5s），或话题长时间无数据，FOLLOW guard 失效，恢复战术层决策。
- **关键参数**:
    | 参数名 | 默认值 | 说明 |
    |--------|--------|------|
    | `enable_follow` | false | 是否启用跟随模式 |
    | `target_topic` | `vision/target_body` | TrackGoal 订阅话题 |
    | `follow_standoff_dist` | 1.5 | 目标 standoff 距离 (m) |
    | `follow_deadband` | 0.2 | 进/退切换死区 (m) |
    | `follow_staleness_sec` | 0.5 | 目标失效超时 (s) |
    | `map_frame` | `map` | 全局规划坐标系 |
    | `chassis_frame` | `chassis` | 车体坐标系 |
- **串口侧**: 发布 `TrackGoal` 的串口 RX 逻辑待 MCU/串口侧实现。
- **离线测试**: `tests/follow_mock.py`（mock 30Hz 移动目标 + `map→chassis` TF）+ `tests/follow_smoke.sh`（验证 standoff 距离、目标追踪、失效回落）。
