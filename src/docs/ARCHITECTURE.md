...
### 1.1 项目背景与功能描述
本项目是为 RoboMaster 哨兵机器人量身定制的自主导航系统。哨兵机器人在比赛中承担着全自动防御和进攻的核心任务，要求系统具备极高的定位精度、环境感知能力以及复杂的战术决策逻辑。本系统集成了从底层传感器驱动到高层行为树决策的全栈功能，支持 Gazebo Harmonic 仿真与真实机器人两种模式，确保机器人在动态变化的赛场环境中能够稳定运行。
...
[裁判系统 (Referee)] ----> [BehaviorTree.CPP]  <----+      [Velocity Smoother]
                           (高层战术决策)                           |
                                               ...
                                               +--------------------+---------------------+
                                               |                                          |
                                      [rm_serial_driver]                    [Gazebo MecanumDrive2]
                                         (实车底盘)                              (仿真底盘)
```
...
- **仿真平台**: Gazebo Harmonic (rmuc_2025 / rmuc_2026 / rmul_2026)
...
仿真和实车均使用 `nav2_smac_planner::SmacPlanner2D`，即基于 A* 的 2D 网格规划器。全向底盘不需要考虑最小转弯半径约束，SmacPlanner2D 在全向移动场景下路径更简洁、计算开销更低。
...
- **功能**: 最终的指令转换层。
- **坐标补偿**: Nav2 在 `gimbal_yaw_fake`（不随云台旋转的虚拟系）下规划直线，避免云台自旋时 Nav2 看到底盘"原地打转"。该模块根据 TF 实时查询 `gimbal_yaw_fake → gimbal_yaw` 的相对偏转角，把 `cmd_vel_nav2_result` 的线速度旋回真实的 `gimbal_yaw` 底盘系，输出到 `/cmd_vel_chassis` (`geometry_msgs/Twist`)，实车由 `rm_serial_driver` 订阅，仿真则由 `spawn_robots.launch.py` 的 `robot_base` 节点 remap 到 Gazebo `MecanumDrive2` 插件。
...
- **关键参数**: `cmd_spin_topic: "cmd_spin"`、`init_spin_speed: 0.0`（YAML 默认 0；仿真中保持 0 即可，实车比赛中典型值约 3.14 rad/s，由上层通过 `/cmd_spin` 持续推送）。`fake_robot_base_frame: "gimbal_yaw_fake"`、`robot_base_frame: "gimbal_yaw"` 决定坐标补偿的源/目标系。
...
### 7.1 仿真环境概述
系统基于 Gazebo Harmonic 构建了比赛场地仿真，支持三种世界：`rmuc_2025`、`rmuc_2026`、`rmul_2026`。仿真栈位于 `src/simulator/`，含 `rmoss_gazebo`（gz 基础插件）、`rmoss_gz_resources`（机器人模型与世界 SDF）、`rmu_gazebo_simulator`（一键 bringup）等包。

- **物理模型**: 标准机器人使用 `rmua19_standard_robot` 模型，挂载 `MecanumDrive2` 驱动四麦轮运动，装配 `gpu_lidar` 仿真 Livox Mid360。
- **传感器模拟**: `gpu_lidar` 输出 `PointCloudPacked` 格式，由 `ign_sim_pointcloud_tool` 节点转换为标准 `PointCloud2`（话题 `velodyne_points`）供 Point-LIO 处理。Point-LIO 仿真配置需将 `timestamp_unit` 设为 `0`（秒），因为 `ign_sim_pointcloud_tool` 写入的每点时间戳单位是秒。
- **速度接入**: 仿真机器人通过 `spawn_robots.launch.py` 中的 `robot_base` 节点，把 ROS2 话题 `/cmd_vel_chassis` remap 进 Gazebo `MecanumDrive2` 插件。与实车的 `/cmd_vel_chassis → rm_serial_driver` 链路在 fake_vel_transform 以上完全相同。

### 7.2 两步启动（时序敏感）

```bash
# 终端 1：启动 Gazebo（Wayland 下加 QT_QPA_PLATFORM=xcb）
...
> **注意**：必须先 unpause，等仿真时钟稳定后再启导航栈。过早启动会导致 Point-LIO 时间戳异常（`lidar loop back, clear buffer`）。仿真中 `init_spin_speed` 应保持 `0.0`。

### 7.3 仿真与实车的关键差异

| 组件 | 仿真 | 实车 |
|------|------|------|
| 速度指令终点 | `MecanumDrive2` (Gazebo, 经 remap) | `rm_serial_driver` |
| LiDAR 话题 | `velodyne_points` (经 ign_sim_pointcloud_tool 转换) | `livox/lidar` |
| Point-LIO `timestamp_unit` | `0` (秒) | `3` (纳秒) |
| `init_spin_speed` | `0.0` | `3.14` rad/s (典型值) |
| TF 发布者 | Gazebo (robot_state_publisher) | robot_state_publisher + rm_serial_driver |

### 7.4 调试工具
- **RViz2**: 预设完整配置，可查看代价地图、全局/局部路径、实时点云、TF 树及重定位状态。
- **Groot2 / btviz**: 通过 ZMQ 协议连接行为树服务器（默认端口 1667），实时查看节点执行状态与黑板变量。
- **Wayland 适配**: Ubuntu 24.04 Wayland 下 Gazebo GUI Play 按钮可能失效，启动前设置 `export QT_QPA_PLATFORM=xcb`，或用命令行 unpause（见 7.2 节）。
