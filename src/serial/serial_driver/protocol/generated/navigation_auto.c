...
{
  static uint8_t  ring[APP_RX_DATA_SIZE];
  static uint32_t fill = 0;
  if (data == 0 || len == 0) return;
  if (fill + len > APP_RX_DATA_SIZE) fill = 0;  /* overflow → reset */
  memcpy(&ring[fill], data, len);
  fill += len;

  uint32_t i = 0;
  while (i + 2 <= fill) {
    uint8_t hdr = ring[i];
    /* Quick reject: header must be one of ours (RX-from-ROS range 0xA0..0xAF). */
    if (hdr < 0xA0 || hdr > 0xAF) {
      i += 1;
      continue;
    }
    uint8_t plen = ring[i + 1];
    uint32_t frame_len = (uint32_t)plen + 4;
    if (i + frame_len > fill) break;  /* incomplete frame, wait for more */
    parse_frame(&ring[i], frame_len);
    i += frame_len;
  }
  if (i > 0) {
    if (i >= fill) {
