...
#include "sentry_behavior/plugins/condition/is_rfid_detected.hpp"
...
BT::NodeStatus IsRfidDetectedCondition::checkRfidStatus()
{
  ...
  auto msg = getInput<rm_interfaces::msg::RfidStatus>("key_port");
  if (!msg) {
  ...
  if (
    ...
    (friendly_supply_zone_non_exchange &&
     msg->friendly_supply_zone_non_exchange ) ||
...
{
  return {
    BT::InputPort<rm_interfaces::msg::RfidStatus>(
...
{
  factory.registerNodeType<sentry_behavior::IsRfidDetectedCondition>("IsRfidDetected");
}
