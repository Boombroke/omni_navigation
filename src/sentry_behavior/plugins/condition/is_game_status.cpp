...
#include "sentry_behavior/plugins/condition/is_game_status.hpp"
...
BT::NodeStatus IsGameStatusCondition::checkGameStart()
{
  ...
  auto msg = getInput<rm_interfaces::msg::GameStatus>("key_port");
  if (!msg) {
...
{
  return {
    BT::InputPort<rm_interfaces::msg::GameStatus>(
...
{
  factory.registerNodeType<sentry_behavior::IsGameStatusCondition>("IsGameStatus");
}
