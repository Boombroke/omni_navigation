...
3. 将线速度从 `gimbal_yaw_fake` 系旋转到 `gimbal_yaw` 系（2D 旋转矩阵），解耦云台独立瞄准
4. angular.z 直通（wz_out = wz_in），物理自旋由 MCU 固件侧负责
...
| 话题 | 类型 | 说明 |
|------|------|------|
| `output_cmd_vel_topic` | `geometry_msgs/msg/Twist` | 坐标旋转后的底盘速度指令 |
